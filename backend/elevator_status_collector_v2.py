...
from app.core.data_paths import STATION_FACILITIES_FILE, STATION_REALTIME_CURRENT_DIR
from app.station_config.station_metadata import get_station_file_slug
...
SERVICE_KEY = os.getenv("ELEVATOR_SERVICE_KEY", "").strip()
...
LOCATION_FILE = STATION_FACILITIES_FILE
...
def find_location_file() -> Path | None:
    """
    station_facilities.json을 자동으로 찾습니다.
    ...
    filename = "station_facilities.json"
...
def save_to_json(
    ...
    if filename is None:
        filename = f"{get_station_file_slug(building_name)}_equipment_status.json"

    STATION_REALTIME_CURRENT_DIR.mkdir(parents=True, exist_ok=True)
    output_path = STATION_REALTIME_CURRENT_DIR / filename
